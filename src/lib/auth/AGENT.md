...
- **Auth locale boilerplate** — exception bornée : en développement uniquement,
  ...
  sans DB. Quand `DATABASE_URL` existe aussi en dev, Better Auth reste disponible
  pour les comptes DB créés localement. Le founder peut changer de perspective
  dashboard via le switch Founder/Admin, mais il n'existe pas de second compte
  admin local sans DB. Ne pas utiliser pour un projet client, staging ou
  production.
